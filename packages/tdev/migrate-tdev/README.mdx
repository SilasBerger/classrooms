...
```bash
yarn workspace @tdev/migrate-tdev migrate [[--only="inf-abc,inf-ccd"]] [[--skip="inf-abc,inf-ccd"]] [[--done]] [[--branch="branch-name"]]
```
...
`--branch`
: Gibt den Branch an, der im Remote-Projekt vor dem Ausführen der Aktion ausgecheckt werden soll (Standard: __main__). Der Branch muss existieren.
...
Eine Aktion ist wie eine Migration, aber sie kann beliebig oft ausgeführt werden und wird nicht in `.done.ts` umbenannt. Aktionen werden im Ordner `actions` abgelegt und können mit folgendem Befehl ausgeführt werden:

```bash
yarn workspace @tdev/migrate-tdev runAction <action-name> [[--only="inf-abc,inf-ccd"]] [[--skip="inf-abc,inf-ccd"]] [[--branch="branch-name"]]
```

Einige Aktionen sind bereits vorhanden:

`format`
: formatiert das Projekt mit `yarn format` und committet+pushed die Änderungen.
`updateTdev`
: führt `yarn run updateTdev` aus, installiert mögliche Package-Updates, formatiert die Codebasis und committet+pushed die Änderungen.
`reinstall`
: entfernt `node_modules` und `yarn.lock`, installiert alle Pakete neu und committet+pushed die Änderungen.
`git.status`
: zeigt den aktuellen Git-Status an.
...
const migrate: MigrationRunner = async (root): Promise<void> => {
    // alle Dateien im Projekt, die den String "@observableRef" enthalten.
...
## Migrationen und Actions

Migrationen tdev
: Siehe [@github/migrations](https://github.com/GBSL-Informatik/teaching-dev/tree/main/packages/tdev/migrate-tdev/migrations)
Actions
: Siehe [@github/actions](https://github.com/GBSL-Informatik/teaching-dev/tree/main/packages/tdev/migrate-tdev/actions)
